...
static constexpr float CAR_EYE_ROTATION = 10.0f * static_cast<float>(M_PI) / 180.0f;
static constexpr float CAR_TURN_RATE = 180.0f * static_cast<float>(M_PI) / 180.0f;
static constexpr float CAR_ACCEL_RATE = 3.0f;

static constexpr float CAR_VELOCITY_DECAY = 0.5f; // per second
...
static constexpr float TRACK_MIN_WIDTH = 2.0f;
static constexpr float TRACK_MAX_WIDTH = 4.0f;
static constexpr float TRACK_MAX_SKEW = 3.0f;
